...
public class GpsLogCompatibilityController {

	private final GpsTelemetryService gpsTelemetryService;

	public GpsLogCompatibilityController(GpsTelemetryService gpsTelemetryService) {
		this.gpsTelemetryService = gpsTelemetryService;
	}

	@PostMapping("/log")
	@ResponseStatus(HttpStatus.CREATED)
	public GpsTelemetryResponse saveLog(@RequestBody GpsTelemetryRequest request) {
		return gpsTelemetryService.save(request);
	}

	@GetMapping("/logs")
	public List<GpsTelemetryResponse> logs() {
