"""
[OCR Builder v3 — Layout filter 추가 (BUGGY)]

v2 → v3 변경:
  - per-char bbox 검사 추가: 한글이 모든 숫자보다 x축 뒤(우측) 에 있어야 함
  - 의도: 2줄 plate 자동 거부

⚠️ 버그:
  - char_list 가 frame 안 모든 plate 의 char 를 섞어서 사용
  - frame 에 plate 2개 있으면 (예: Plate A 가 왼쪽, Plate B 가 오른쪽):
    Plate A 검사 시 Plate B 의 숫자 x 좌표 까지 max 비교 대상
    → Plate A 가 정상 1줄 plate 인데도 layout_fail 처리
  - 결과: layout_fail 비율 비현실적 (~50%) 로 폭증
"""
...
def main():
    ...
    print(f"[init v3] src={src}\n           out={out}\n           quality={args.quality}")
    print("\n[1/2] 파일 매칭 ...")
    pairs = find_jpg_label_pairs(src)
    print(f"  pairs: {len(pairs):,}")

    print("\n[2/2] 엄격 패턴 plate crop + 라벨 추출 ...")
    print("       허용 패턴: ^\\d{2,3}[가-힣]$ + layout (한글이 숫자 뒤)")
    saved = 0
    n_pattern_fail = 0
    n_layout_fail = 0      # v3 신규
    n_skip_bbox = 0
    n_skip_load = 0
    t0 = time.time()
        ...
        for a in annotations:
            ...
            for lp in a.get("license_plate", []):
                ...
                if not OCR_PAT.match(text):
                    n_pattern_fail += 1
                    continue
                # ⚠️ v3 BUG: char_list 전체 (모든 plate 섞임) 사용
                hg_chars = [c for c in char_list
                             if HANGUL_RE.search(c.get("text", ""))]
                dg_chars = [c for c in char_list
                ...
                if hg_x_min < dg_x_max:
                    # 한글이 어떤 숫자보다 좌측 → 2줄 plate or 비정상
                    # ⚠️ 실제로는 frame 의 다른 plate 숫자가 우측에 있어 잘못 거부
                    n_layout_fail += 1
                    continue
    ...
    print(f"\n=== v3 완료 ({elapsed/60:.1f}분) ===")
    print(f"  scanned:        {len(pairs):,}")
    print(f"  saved:          {saved:,}")
    print(f"  pattern_fail:   {n_pattern_fail:,}")
    print(f"  layout_fail:    {n_layout_fail:,}  ⚠️ 버그로 과도 거부")
