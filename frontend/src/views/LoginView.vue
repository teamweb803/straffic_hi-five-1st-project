...
async function handleSubmit() {
  ...
  if (result.ok) {
    const redirect = route.query.redirect ?? (auth.isMasterAdmin ? '/master-admin' : '/dashboard')
...
<template>
  <section class="min-h-[80vh] flex items-center justify-center bg-cloud px-6 py-16">
    <div class="w-full max-w-md card shadow-sm">
      ...
      <p class="text-sm text-navy/70 mt-2">HI-FIVE 관리자/관제 대시보드에 접속합니다.</p>

      <form class="mt-8 space-y-4" @submit.prevent="handleSubmit">
        <div>
          <label class="block text-xs font-semibold text-navy/70 mb-1">이메일</label>
          <input v-model.trim="form.email" type="email" autocomplete="username" class="input-field" placeholder="admin@hifive.com" />
        </div>
        <div>
          <label class="block text-xs font-semibold text-navy/70 mb-1">비밀번호</label>
          <input v-model="form.password" type="password" autocomplete="current-password" class="input-field" placeholder="비밀번호" />
        </div>
