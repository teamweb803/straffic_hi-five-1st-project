...
function fmtTime(v) { if (!v) return '-'; return new Date(v).toLocaleTimeString('ko-KR', { hour12: false }) }
      ...
      <main>
        ...
        <section class="widget-grid">
          <article v-for="(w, idx) in widgets" :key="w.id"
            class="widget panel"
            :class="{ dragging: draggedIdx === idx, editing: editMode }"
                ...
                <table class="log-table">
                  ...
                  <tbody>
                    <tr v-for="log in gpsTelemetry" :key="log.id">
                      <td class="mono">{{ fmtTime(log.capturedAt) }}</td>
...
.widget{position:relative;display:flex;flex-direction:column;gap:14px;min-width:0;transition:opacity .15s,transform .15s}
...
.gps-log-fill{flex:1;min-height:0;overflow-y:auto;border-radius:8px;border:1px solid rgba(56,120,245,.14);background:rgba(2,9,22,.4)}
